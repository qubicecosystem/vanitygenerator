// Coded by Come-from-Beyond 2024 ©
...
static void getIdentity(unsigned char* publicKey, char* identity)
{
    for (int i = 0; i < 4; i++)
    {
        ...
        for (int j = 0; j < 14; j++)
        {
            identity[i * 14 + j] = publicKeyFragment % 26 + 'A';
    ...
    for (int i = 0; i < 4; i++)
    {
        identity[56 + i] = identityBytesChecksum % 26 + 'A';
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        while (true)
        {
            ...
            unsigned char identity[60 + 1];
            getIdentity(publicKey, (char*)identity);
            bool found = true;
            for (int i = 0; i < strlen(argv[1]); i++)
            {
                if (identity[i] != argv[1][i])
                {
                    found = false;
